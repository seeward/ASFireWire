...
#include "DuplexStreamPlanner.hpp"
...
void LogReservationRefusal(const char* direction, uint32_t streamIndex,
                           ...
                           const Duplex::IRMReservationResult& reservation) noexcept {
    ASFW_LOG_ERROR(Audio,
                   "IRM reserve REFUSED %{public}s[%u] cause=%{public}s status=0x%08x "
                   "slots=%u packet=%u overhead=%u total=%u available=%u gap=%u "
                   "allowed=0x%016llx refused=0x%016llx endpoint=%llx gen=%u",
                   direction, streamIndex, Duplex::ToString(reservation.failure),
                   ...
                   reservation.refusedChannels, endpointId.value, generation.value);
}
...
[[nodiscard]] AudioClockConfig EffectiveStartClockForProfile(
    ...
    for (uint8_t i = 0; i < profile.supportedRateCount; ++i) {
        if (profile.supportedRates[i] == requestedClock.sampleRateHz) {
            return requestedClock;
        }
    }
    return AudioClockConfig{.sampleRateHz = profile.currentSampleRateHz};
}

[[nodiscard]] Devices::ResolvedAudioEndpointProfile WithRuntimeCaps(
    const Devices::ResolvedAudioEndpointProfile& profile,
    const AudioStreamRuntimeCaps& caps) {
    auto current = profile;
    current.runtimeCaps = caps;
    if (caps.sampleRateHz != 0) current.currentSampleRateHz = caps.sampleRateHz;
    return current;
}
...
public:
    struct Dependencies {
        ...
        Duplex::DuplexOperationGate& gate;
        Duplex::RestartSessionStore& sessionStore;
...
private:
    ...
    [[nodiscard]] bool IsRestartEpochCurrent(EndpointId endpointId, uint64_t restartId,
    ...
    [[nodiscard]] Runtime::IDirectAudioBindingSource* GetDirectAudioBindingSource(
        EndpointId endpointId) const noexcept;
...
void DuplexStartTransaction::RecordTeardownAbort(const char* stage, EndpointId endpointId) noexcept {
    ...
    ASFW_LOG(Audio,
             ...
             "endpoint=%llx kr=0x%x",
             stage ? stage : "unknown", endpointId.value, kIOReturnAborted);
}

bool DuplexStartTransaction::IsStopRequested(EndpointId endpointId) const noexcept {
    return dependencies_.gate.IsStopRequested(endpointId);
}

bool DuplexStartTransaction::IsRestartEpochCurrent(
    EndpointId endpointId, uint64_t restartId, const Discovery::DeviceRouteToken& route) const noexcept {
    if (!endpointId || restartId == 0 || !route || IsStopRequested(endpointId)) {
        return false;
    }

    const DuplexRestartSession session = dependencies_.sessionStore.LoadSession(endpointId);
    if (session.restartId != restartId || session.route != route ||
        !dependencies_.registry.IsCurrent(route)) {
        return false;
    }
    const auto profile = dependencies_.runtime.FindProfile(endpointId);
    return profile && route.deviceInstanceId == profile->deviceInstanceId;
}

Runtime::IDirectAudioBindingSource* DuplexStartTransaction::GetDirectAudioBindingSource(
    EndpointId endpointId) const noexcept {
    return dependencies_.bindingSourceProvider
               ? dependencies_.bindingSourceProvider(endpointId)
               : nullptr;
}
...
IOReturn AudioDuplexCoordinator::StartStreaming(EndpointId endpointId) noexcept {
    if (!endpointId) {
        return kIOReturnBadArgument;
    }
    if (TeardownRequested()) {
        ASFW_LOG(Audio,
                 "AudioDuplexCoordinator: StartStreaming refused by teardown endpoint=%llx",
                 endpointId.value);
    ...
    LogFsmEvent("start", endpointId, session.restartId, session.topologyGeneration, session.state,
                session.phase, session.reason);

    bool acquired = false;
    for (uint32_t waited = 0; waited < kSyncBridgeTimeoutMs; waited += kSyncBridgePollMs) {
        if (TryAcquireEndpoint(endpointId)) {
            acquired = true;
            break;
        }
        if (TeardownRequested()) {
    ...
    if (!acquired) {
        ASFW_LOG_ERROR(Audio,
                       "AudioDuplexCoordinator: StartStreaming timed out waiting for endpoint "
                       "claim endpoint=%llx timeoutMs=%u",
                       endpointId.value, kSyncBridgeTimeoutMs);
        return kIOReturnTimeout;
    }

    const IOReturn status = RunStartStreaming(endpointId);
    ReleaseEndpoint(endpointId);
    return status;
}

IOReturn AudioDuplexCoordinator::StopStreaming(EndpointId endpointId) noexcept {
    if (!endpointId) {
        return kIOReturnBadArgument;
    }
    if (TeardownRequested()) {
        ASFW_LOG(Audio, "AudioDuplexCoordinator: StopStreaming refused by teardown endpoint=%llx",
                 endpointId.value);
        return kIOReturnAborted;
    }

    const DuplexRestartSession session = LoadSession(endpointId);
    LogFsmEvent("stop", endpointId, session.restartId, session.topologyGeneration, session.state,
                session.phase, session.reason);

    RequestStopIntent(endpointId);
    FailPendingClockRequest(endpointId, DuplexClockRequestOutcome::kAbortedByStop, kIOReturnAborted);
    ...
    if (!acquired) {
        ASFW_LOG_ERROR(Audio,
                       "AudioDuplexCoordinator: StopStreaming timed out waiting for endpoint "
                       "claim endpoint=%llx timeoutMs=%u",
                       endpointId.value, kSyncBridgeTimeoutMs);
        ClearStopIntent(endpointId);
        return kIOReturnTimeout;
    ...
    const IOReturn status = RunStopStreaming(endpointId);
    FailPendingClockRequest(endpointId, DuplexClockRequestOutcome::kAbortedByStop, kIOReturnAborted);
    ClearStopIntent(endpointId);
    ReleaseEndpoint(endpointId);
    return status;
...
IOReturn AudioDuplexCoordinator::RequestClockConfig(
    EndpointId endpointId, const AudioClockConfig& desiredClock,
    ...
    if (TeardownRequested()) {
        ASFW_LOG(Audio,
                 "AudioDuplexCoordinator: RequestClockConfig refused by teardown endpoint=%llx",
                 endpointId.value);
        return kIOReturnAborted;
    }
    ...
    const auto resolvedProfile = runtime_.FindProfile(endpointId);
    if (!resolvedProfile) return kIOReturnNotReady;
    bool supportedByEndpoint = false;
    for (uint8_t i = 0; i < resolvedProfile->supportedRateCount; ++i) {
        supportedByEndpoint = supportedByEndpoint ||
                              resolvedProfile->supportedRates[i] == desiredClock.sampleRateHz;
    }
    if (!supportedByEndpoint) return kIOReturnUnsupported;
    ...
    IOLockLock(lock_);
    if (gate_.IsStopRequestedLocked(endpointId)) {
        IOLockUnlock(lock_);
        return kIOReturnAborted;
    }

    request.token = clockRequests_.AllocateTokenLocked();
    if (gate_.IsActiveLocked(endpointId)) {
        supersededRequest = clockRequests_.QueuePendingLocked(endpointId, request);
    } else {
        gate_.AcquireLocked(endpointId);
    ...
    LogFsmEvent("clock", endpointId, session.restartId, session.topologyGeneration, session.state,
...
IOReturn AudioDuplexCoordinator::RecoverStreaming(EndpointId endpointId,
                                                   DuplexRestartReason reason) noexcept {
    if (!endpointId) {
        return kIOReturnBadArgument;
    }
    if (TeardownRequested()) {
        ASFW_LOG(Audio,
                 "AudioDuplexCoordinator: RecoverStreaming refused by teardown endpoint=%llx",
                 endpointId.value);
        return kIOReturnAborted;
    }
    if (IsStopRequested(endpointId)) {
    ...
    if (!acquired) {
        ASFW_LOG_ERROR(Audio,
                       "AudioDuplexCoordinator: RecoverStreaming timed out waiting for endpoint "
                       "claim endpoint=%llx timeoutMs=%u",
                       endpointId.value, kSyncBridgeTimeoutMs);
        return kIOReturnTimeout;
    }

    const IOReturn status = RunRecoveryStreaming(endpointId, reason);
    ReleaseEndpoint(endpointId);
    return status;
}

void AudioDuplexCoordinator::ClearSession(EndpointId endpointId) noexcept {
    if (!lock_ || !endpointId) {
        return;
    }

    IOLockLock(lock_);
    store_.EraseSessionLocked(endpointId);
    clockRequests_.ClearLocked(endpointId);
    gate_.ReleaseLocked(endpointId);
    gate_.ClearStopLocked(endpointId);
    IOLockUnlock(lock_);
}

void AudioDuplexCoordinator::CancelRemoteDevice(EndpointId endpointId) noexcept {
    if (!endpointId) {
        return;
    }
    gate_.MarkRemoteDeviceLost(endpointId);
    FailPendingClockRequest(endpointId, DuplexClockRequestOutcome::kAbortedByStop,
                            kIOReturnAborted);
}

void AudioDuplexCoordinator::AcknowledgeDevicePresent(EndpointId endpointId) noexcept {
...
AudioDuplexCoordinator::GetSession(EndpointId endpointId) const noexcept {
    return store_.GetSession(endpointId);
}

bool AudioDuplexCoordinator::IsOperationInFlight(EndpointId endpointId) const noexcept {
    if (!lock_ || !endpointId) {
        return false;
    }
    IOLockLock(lock_);
    const bool inFlight =
        gate_.IsActiveLocked(endpointId) || clockRequests_.HasPendingLocked(endpointId);
    IOLockUnlock(lock_);
    return inFlight;
}

IOReturn AudioDuplexCoordinator::RunStartStreaming(EndpointId endpointId) noexcept {
    if (TeardownRequested()) {
        return kIOReturnAborted;
    }
    ...
    auto record = RequireDuplexRecord(endpointId, deviceControl, protoHold);
    if (!record || !deviceControl) {
        FailPendingClockRequest(endpointId, DuplexClockRequestOutcome::kFailed, kIOReturnNotReady);
    ...
    const IOReturn status =
        RunDuplexStart(endpointId, *record, *deviceControl, session, desiredClock, reason);
    if (status != kIOReturnSuccess) {
        FailPendingClockRequest(endpointId, DuplexClockRequestOutcome::kFailed, status);
        return status;
    }
    ...
    while (TryConsumePendingClockRequest(endpointId, pending)) {
        if (IsStopRequested(endpointId)) {
            const DuplexRestartSession completionSession = LoadSession(endpointId);
            CompleteClockRequest(
                DuplexClockRequestCompletion{
                    .token = pending.token,
        ...
        const IOReturn pendingStatus = ApplyClockRequest(endpointId, pending);
        if (IsStopRequested(endpointId)) {
            const DuplexRestartSession completionSession = LoadSession(endpointId);
            CompleteClockRequest(
                DuplexClockRequestCompletion{
...
IOReturn AudioDuplexCoordinator::RunStopStreaming(EndpointId endpointId) noexcept {
    if (TeardownRequested()) {
        return kIOReturnAborted;
    }
    ...
    const std::optional<DuplexRestartSession> existingSession = GetSession(endpointId);
    ...
    DuplexRestartSession session = existingSession.value_or(LoadSession(endpointId));
    return RunDuplexStop(endpointId, *record, *deviceControl, session);
}

IOReturn AudioDuplexCoordinator::RunRecoveryStreaming(EndpointId endpointId,
                                                            DuplexRestartReason reason) noexcept {
    if (TeardownRequested()) {
        return kIOReturnAborted;
    }
    ...
    const RecoveryContext context{
        ...
        .state = session.state,
        .phase = session.phase,
        .stopRequested = IsStopRequested(endpointId),
    ...
    if (decision.disposition == RecoveryDisposition::kIgnore) {
        ...
        return (decision.reason == RecoveryPolicyReason::kSuppressedByStop ||
                decision.reason == RecoveryPolicyReason::kIdleApplyInvalidated)
                   ? kIOReturnAborted
                   : kIOReturnUnsupported;
    }

    if (decision.disposition == RecoveryDisposition::kFailSession) {
    ...
    if (HasAnyRestartState(session) || session.phase == DuplexRestartPhase::kRunning) {
        ...
        const IOReturn stopStatus = RunDuplexStop(endpointId, *record, *deviceControl, session);
        if (stopStatus != kIOReturnSuccess) {
            return stopStatus;
        }
    }

    if (IsStopRequested(endpointId) || TeardownRequested()) {
        return kIOReturnAborted;
    }

    return RunDuplexStart(endpointId, *record, *deviceControl, session, desiredClock, reason);
}

IOReturn
AudioDuplexCoordinator::RunClockRequestLoop(EndpointId endpointId,
    ...
    while (true) {
        ...
        lastStatus = ApplyClockRequest(endpointId, current);
        ...
        if (!TryConsumePendingClockRequest(endpointId, next)) {
            break;
        }
        current = next;
    }
...
IOReturn
AudioDuplexCoordinator::ApplyClockRequest(EndpointId endpointId,
    ...
    if (HasAnyRestartState(session) || session.phase == DuplexRestartPhase::kRunning ||
        ...
        return RunDuplexStart(endpointId, *record, *deviceControl, session, request.desiredClock,
                              request.reason);
    }
    ...
    return RunIdleClockApply(endpointId, *deviceControl, session, record->gen, request.desiredClock,
                             request.reason);
}

IOReturn DuplexStartTransaction::Run(const StartRequest& request) noexcept {
    const EndpointId endpointId = request.endpointId;
    ...
    const auto resolvedProfile = dependencies_.runtime.FindProfile(endpointId);
    if (!resolvedProfile) {
        return kIOReturnNotReady;
    }
    const AudioClockConfig desiredClock =
        EffectiveStartClockForProfile(*resolvedProfile, request.desiredClock);
    ...
    auto RecordTeardownAbort = [this](const char* stage, EndpointId abortEndpoint) noexcept {
        this->RecordTeardownAbort(stage, abortEndpoint);
    };
    ...
    auto IsStopRequested = [this](EndpointId requestEndpoint) noexcept {
        return this->IsStopRequested(requestEndpoint);
    };
    auto IsRestartEpochCurrent = [this](EndpointId requestEndpoint, uint64_t restartId,
                                        const Discovery::DeviceRouteToken& route) noexcept {
        return this->IsRestartEpochCurrent(requestEndpoint, restartId, route);
    };
    auto GetDirectAudioBindingSource = [this](EndpointId requestEndpoint) noexcept {
        return this->GetDirectAudioBindingSource(requestEndpoint);
    };
    auto RunDuplexStop = [this](EndpointId stopEndpoint, Discovery::DeviceRecord& stopRecord,
                                IDuplexDeviceControl& stopDevice,
                                DuplexRestartSession& stopSession) noexcept {
        return Stop(StopRequest{stopEndpoint, stopRecord, stopDevice, stopSession});
    };
    ...
    auto abortIfTeardown = [&](const char* stage) noexcept -> bool {
        ...
        RecordTeardownAbort(stage, endpointId);
        return true;
    };
    ...
    const auto route = dependencies_.registry.CurrentRoute(record.instanceId);
    if (!route.has_value() || !dependencies_.registry.IsCurrent(*route)) {
        return kIOReturnNotReady;
    }
    const FW::Generation topologyGeneration = route->generation;
    auto runtimeProtocol = runtime_.FindShared(endpointId);
    ...
    // caps that StreamPlanner consumes via GetRuntimeAudioStreamCaps.
    ...
    auto rollbackToFailure = [&](IOReturn failureStatus, DuplexRestartPhase failedPhase,
        ...
        (void)WaitForAsyncResult<bool>(
            ...
            kSyncBridgeTimeoutMs, kIOReturnTimeout, cancel_);
        const IOReturn rollbackStatus = RunDuplexStop(endpointId, record, deviceControl, session);
    ...
    auto rollbackToInvalidation = [&](IOReturn invalidationStatus, DuplexRestartPhase failedPhase,
        ...
        RecordIssue(session, session.lastInvalidation, failedPhase,
                    IsStopRequested(endpointId) ? DuplexRestartErrorClass::kStopIntent
    ...
    if (irmClient == nullptr) {
        ASFW_LOG_ERROR(Audio, "AudioDuplexCoordinator: device control missing IRM client endpoint=%llx",
                       endpointId.value);
    ...
    auto* bindingSource = GetDirectAudioBindingSource(endpointId);
    if (!bindingSource) {
        return finalizeFailure(kIOReturnNotReady, DuplexRestartPhase::kPreparingDevice,
                               DuplexRestartFailureCause::kPrepare,
                               DuplexRestartErrorClass::kMissingDependency, false, kIOReturnSuccess,
    ...
    StoreSession(session);
    LogFsmEvent("start", endpointId, restartId, topologyGeneration, session.state,
                session.phase, reason);
    ASFW_LOG(Audio, "AudioDuplexCoordinator: using isoch channels d2h=%u h2d=%u endpoint=%llx",
             channels.deviceToHostIsoChannel, channels.hostToDeviceIsoChannel, endpointId.value);

    if (abortIfTeardown("BeginSplitDuplex")) {
        return kIOReturnAborted;
    }
    const kern_return_t claimStatus = hostTransport_.BeginSplitDuplex(endpointId);
    ...
    for (uint32_t i = 0; i < channels.playbackStreamCount; ++i) {
        const Duplex::PlaybackStreamGeometry& geometry = streamProfile.playbackStreams[i];
        Duplex::IRMReservationResult reservation{};
        const kern_return_t reservePlaybackStatus = hostTransport_.ReservePlaybackResources(
            endpointId, *irmClient, geometry.allowedIsoChannels, geometry.packetBandwidthUnits,
            reservation);
        if (reservePlaybackStatus != kIOReturnSuccess) {
            LogReservationRefusal("playback", i, endpointId, topologyGeneration, geometry.am824Slots,
    ...
    for (uint32_t i = 0; i < channels.captureStreamCount; ++i) {
        const Duplex::CaptureStreamGeometry& geometry = streamProfile.captureStreams[i];
        Duplex::IRMReservationResult reservation{};
        const kern_return_t reserveCaptureStatus = hostTransport_.ReserveCaptureResources(
            endpointId, *irmClient, geometry.allowedIsoChannels, geometry.packetBandwidthUnits,
            reservation);
        if (reserveCaptureStatus != kIOReturnSuccess) {
            LogReservationRefusal("capture", i, endpointId, topologyGeneration, geometry.am824Slots,
                                  geometry.allowedIsoChannels, reservation);
    ...
    ASFW_LOG(Audio,
             ...
             masterCapture.packetBandwidthUnits, masterPlayback.pcmChannels, masterPlayback.am824Slots,
             masterPlayback.isoChannel, masterPlayback.packetBandwidthUnits,
    ...
    for (const Duplex::HostDirection direction : streamProfile.startOrder.prepareOrder) {
        if (direction == Duplex::HostDirection::kReceive) {
            ...
            for (uint32_t i = 1; i < channels.captureStreamCount; ++i) {
                const Duplex::CaptureStreamGeometry& captureStream = streamProfile.captureStreams[i];
    ...
    if (streamProfile.startOrder.requiresPreStreamClockLock) {
        ...
        const IOReturn clockLockStatus =
            WaitForStableGlobalClock(endpointId, deviceControl, topologyGeneration, desiredClock);
        ...
        ASFW_LOG(Audio,
                 "AudioDuplexCoordinator: skipping pre-stream clock lock for BeBoB endpoint=0x%016llx",
                 endpointId.value);
    }
...
IOReturn DuplexStartTransaction::WaitForStableGlobalClock(
    EndpointId endpointId, IDuplexDeviceControl& deviceControl, const FW::Generation topologyGeneration,
    const AudioClockConfig& desiredClock) noexcept {
    const std::atomic<bool>* const cancel_ = dependencies_.cancel;
    ...
    while (UptimeMilliseconds() < deadlineMs) {
        if (TeardownRequested()) {
            RecordTeardownAbort("WaitingGlobalClock", endpointId);
            return kIOReturnAborted;
        }
...
IOReturn DuplexStartTransaction::Stop(const StopRequest& request) noexcept {
    ...
    if (TeardownRequested()) {
        RecordTeardownAbort("RunDuplexStop", endpointId);
        return kIOReturnAborted;
    }
    ...
    if (profile.stopOrder
        ...
        // cleanup (reserved profile + active endpoint) without another wire action.
        ...
        if (result != kIOReturnSuccess) {
            ASFW_LOG_ERROR(Audio,
                           "RunDuplexStop: failed endpointId=0x%016llx tx=0x%08x rx=0x%08x "
                           "cleanup=0x%08x -> 0x%08x",
                           endpointId.value, transmitStopStatus, receiveStopStatus, cleanupStatus, result);
        }
    } else {
        ...
        if (deviceStatus == kIOReturnAborted && TeardownRequested()) {
            RecordTeardownAbort("DeviceStop", endpointId);
            return kIOReturnAborted;
        }
...
IOReturn DuplexStartTransaction::ApplyIdleClock(const IdleClockApplyRequest& request) noexcept {
    ...
    if (TeardownRequested()) {
        RecordTeardownAbort("IdleClockApply", endpointId);
        return kIOReturnAborted;
    }
    ...
    const auto route = dependencies_.registry.CurrentRoute(profile->deviceInstanceId);
    if (!route.has_value() || !dependencies_.registry.IsCurrent(*route)) {
        return kIOReturnNotReady;
    }
...
IOReturn AudioDuplexCoordinator::RunDuplexStart(
    EndpointId endpointId, Discovery::DeviceRecord& record, IDuplexDeviceControl& deviceControl,
    ...
    return transaction.Run(
        DuplexStartTransaction::StartRequest{endpointId, record, deviceControl, session, desiredClock,
                                             reason});
}
...
IOReturn AudioDuplexCoordinator::RunDuplexStop(
    ...
    return transaction.Stop(
        DuplexStartTransaction::StopRequest{endpointId, record, deviceControl, session});
}

IOReturn AudioDuplexCoordinator::RunIdleClockApply(
    EndpointId endpointId, IDuplexDeviceControl& deviceControl, DuplexRestartSession& session,
    ...
    return transaction.ApplyIdleClock(
        DuplexStartTransaction::IdleClockApplyRequest{endpointId, deviceControl, session,
                                                       topologyGeneration, desiredClock, reason});
}

std::optional<Discovery::DeviceRecord> AudioDuplexCoordinator::RequireDuplexRecord(
    EndpointId endpointId, IDuplexDeviceControl*& outDeviceControl,
    ...
    auto record = registry_.Snapshot(profile->deviceInstanceId);
    if (!record.has_value() || !Discovery::TryOperationalNodeId(record->nodeId).has_value()) {
        return std::nullopt;
    }

    auto protocol = runtime_.FindShared(endpointId);
    if (!protocol) {
        return std::nullopt;
    }
...
AudioDuplexCoordinator::GetDirectAudioBindingSource(EndpointId endpointId) const noexcept {
    if (!bindingSourceProvider_) {
        return nullptr;
    }
    return bindingSourceProvider_(endpointId);
}

// FW-68: the per-endpoint gating state + logic now lives in DuplexOperationGate (gate_). These
...
bool AudioDuplexCoordinator::TryAcquireEndpoint(EndpointId endpointId) noexcept {
    return gate_.Acquire(endpointId);
}

void AudioDuplexCoordinator::ReleaseEndpoint(EndpointId endpointId) noexcept {
    gate_.Release(endpointId);
}

void AudioDuplexCoordinator::RequestStopIntent(EndpointId endpointId) noexcept {
    gate_.RequestStop(endpointId);
}

void AudioDuplexCoordinator::ClearStopIntent(EndpointId endpointId) noexcept {
    gate_.ClearStop(endpointId);
}

bool AudioDuplexCoordinator::IsStopRequested(EndpointId endpointId) const noexcept {
    return gate_.IsStopRequested(endpointId);
}
...
bool AudioDuplexCoordinator::IsRestartEpochCurrent(
    EndpointId endpointId, uint64_t restartId, const Discovery::DeviceRouteToken& route) const noexcept {
    const auto profile = runtime_.FindProfile(endpointId);
    if (!endpointId || restartId == 0 || !route || !profile ||
        route.deviceInstanceId != profile->deviceInstanceId) {
        return false;
    }
    if (IsStopRequested(endpointId)) {
        return false;
    ...
    IOLockLock(lock_);
    const auto* sessionPtr = store_.FindSessionLocked(endpointId);
...
bool AudioDuplexCoordinator::TryConsumePendingClockRequest(EndpointId endpointId,
                                                                 PendingClockRequest& outRequest) noexcept {
    return clockRequests_.TryConsumePending(endpointId, outRequest);
}
...
bool AudioDuplexCoordinator::TryTakeCompletedClockRequest(
    ...
    return clockRequests_.TryTakeCompleted(endpointId, token, outCompletion);
}
...
void AudioDuplexCoordinator::CompleteClockRequest(const DuplexClockRequestCompletion& completion,
    ...
    clockRequests_.Complete(completion, endpointId);
}

void AudioDuplexCoordinator::FailPendingClockRequest(EndpointId endpointId,
                                                           DuplexClockRequestOutcome outcome,
                                                           IOReturn status) noexcept {
    clockRequests_.FailPending(endpointId, outcome, status);
}

void AudioDuplexCoordinator::RecordTeardownAbort(const char* stage, EndpointId endpointId) noexcept {
...
DuplexRestartSession AudioDuplexCoordinator::LoadSession(EndpointId endpointId) const noexcept {
    return store_.LoadSession(endpointId);
}
