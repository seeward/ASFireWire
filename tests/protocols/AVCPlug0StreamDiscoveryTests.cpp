...
TEST(AVCPlug0StreamDiscoveryTests, ReadsSectionTypeAfterEchoedSectionId) {
    // BridgeCo section-info response operands 7/8 are respectively the
    // echoed one-based section ID and the actual section type. Treating the
    // former as the type converts a MIDI section into PCM and discards the
    // entire channel permutation as invalid.
    ScriptedBeBoBSubmitter submitter({
        {MakeCdb(0x01, 0x02, {0x00, 0x00, 0x00, 0x00, 0x00}),
         AVCResult::kImplementedStable,
         MakeCdb(0x0c, 0x02, {0x00, 0x01, 0x01, 0x00, 0x00})},
        ...
        {MakeCdb(0x01, 0x02, {0xc0, 0x01, 0x00, 0x00, 0x00, 0xff, 0x03}),
         AVCResult::kImplementedStable,
         MakeCdb(0x0c, 0x02, {0xc0, 0x01, 0x00, 0x00, 0x00, 0xff, 0x03,
                               0x01, 0x01, 0x01, 0x01})},
        {MakeCdb(0x01, 0x02, {0xc0, 0x00, 0x00, 0x00, 0x00, 0xff, 0x07, 0x01}),
         AVCResult::kImplementedStable,
         MakeCdb(0x0c, 0x02, {0xc0, 0x00, 0x00, 0x00, 0x00, 0xff, 0x07,
                               0x01, 0x03})},
        {MakeCdb(0x01, 0x02, {0xc0, 0x01, 0x00, 0x00, 0x00, 0xff, 0x07, 0x01}),
         AVCResult::kImplementedStable,
         MakeCdb(0x0c, 0x02, {0xc0, 0x01, 0x00, 0x00, 0x00, 0xff, 0x07,
                               0x01, 0x0a})},
    });

    std::optional<DeviceModel> model;
    StartAVCPlug0Discovery(submitter, 0x000aac0300b1d1f7ULL,
                           [&model](const auto& discovered) { model = discovered; });

    ASSERT_TRUE(submitter.Finished());
    ASSERT_TRUE(model.has_value());
    ASSERT_EQ(model->input.channelSections.size(), 1U);
    ASSERT_EQ(model->output.channelSections.size(), 1U);
    EXPECT_EQ(model->input.channelSections[0].type, 0x03);
    EXPECT_EQ(model->output.channelSections[0].type, 0x0a);
}
